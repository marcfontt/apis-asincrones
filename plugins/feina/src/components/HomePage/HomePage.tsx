...
const STEPS = [
  ...
  { num: 3, color: '#d97706', title: 'Llanca el Benchmark',     desc: 'El sistema desplega automaticament la infraestructura a Kubernetes.',         href: '/escenaris', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> },
  { num: 4, color: '#7c3aed', title: 'Analitza els Resultats',  desc: "Compara latencia, throughput i taxa d'error en temps real.",                  href: '/resultats', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg> },
];
...
const QUICK_SCENARIOS = [
  { name: 'Kafka EDA - Alta Carrega',       architecture: 'EDA', protocol: 'Kafka', platform: 'Kafka',       description: 'Throughput maxim amb Apache Kafka. Ideal per validar limits de capacitat.',    defaults: { duration: 60, rate: 1000, payloadSize: 256 }, accentColor: '#ef4444' },
  { name: 'MQTT IoT - Baix Consum',         architecture: 'EDA', protocol: 'MQTT',  platform: 'RabbitMQ',    description: 'Protocol lleuger per a escenaris IoT. Baix overhead, latencia minima.',        defaults: { duration: 30, rate: 500,  payloadSize: 128 }, accentColor: '#16a34a' },
  { name: 'gRPC - Serialitzacio Binaria',   architecture: 'LCA', protocol: 'gRPC',  platform: 'NATS Server', description: 'Comunicacio RPC asincrona amb serialitzacio binaria.',                        defaults: { duration: 45, rate: 2000, payloadSize: 512 }, accentColor: '#7c3aed' },
...
const DATA_FORMATS = [
  ...
  { key: 'iot',       label: 'IoT',         color: '#10b981', icon: '🌡', desc: 'Simulacio de sensors: temperatura, humitat, estat. Missatges petits i alta frequencia.',                               payload: '64-128 B', rate: 'alt' },
  { key: 'financial', label: 'Financer',    color: '#0ea5e9', icon: '💹', desc: 'Operacions de trading i dades de mercat. Baix volum per missatge, latencia critica.',                                  payload: '512 B',   rate: 'moderat' },
  ...
  { key: 'video-8k',  label: 'Video 8K',    color: '#7c3aed', icon: '🎥', desc: 'Cas extrem de streaming 8K. Mesura la capacitat maxima del sistema en condicions de carrega maxima.', payload: '256 KB',  rate: 'molt baix' },
];
...
export const HomePage = () => {
  ...
  useEffect(() => { document.title = 'Inici | APIs Asincrones'; }, []);
    ...
    <div style={{ ...S.page, paddingTop: 32 }}>
      ...
      <div style={{ marginBottom: 36, padding: '36px 40px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <h1 style={{ margin: '0 0 12px', fontSize: 30, fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1.2, letterSpacing: '-0.02em' }}>
            Mesura el rendiment de les<br /><span style={{ color: 'var(--accent)' }}>APIs Asincrones</span> en temps real
          </h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, margin: '0 0 28px', lineHeight: 1.65, maxWidth: 600 }}>
            Dissenya escenaris de benchmark, desplega infraestructura real sobre Kubernetes i analitza latencia, throughput i fiabilitat.
          </p>
            ...
            {STATS.map((stat, i) => (
              <a key={stat.label} href={stat.href}
                ...
                style={{
                  ...
                  background: hoveredStat === i ? stat.color + '10' : 'transparent',
                  borderColor: hoveredStat === i ? stat.color + '30' : 'transparent',
          ...
          {STEPS.map((step, i) => (
            <a key={step.num} href={step.href}
              ...
              style={{
                ...
                background: hoveredStep === i ? step.color + '06' : 'var(--bg-card)',
          ...
          {QUICK_SCENARIOS.map((sc, i) => (
            <div key={sc.name}
              ...
              style={{
                ...
                background: hoveredScenario === i ? sc.accentColor + '06' : 'var(--bg-card)',
          ...
          {DATA_FORMATS.map((fmt, i) => (
            <div key={fmt.key}
              ...
              style={{
                ...
                background: hoveredFormat === i ? fmt.color + '08' : 'var(--bg-card)',
          ...
          {CATEGORIES.map((cat, i) => (
            <a key={cat.key} href={cat.href}
              ...
              style={{
                ...
                background: hoveredCat === i ? cat.color + '08' : 'var(--bg-card)',
      ...
      <div style={{ padding: '16px 20px', background: 'var(--bg-subtle)', borderRadius: 10, border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
          <strong style={{ color: 'var(--text-primary)' }}>APIs Asincrones</strong> - Plataforma de benchmark per a APIs sobre AKS
        </span>
