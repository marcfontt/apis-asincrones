...
const HistorialTab = () => {
  ...
  const syncedSummary  = runs.length > 0
    ? summary.filter(s => runScenarioIds.has(s.scenarioId))
    : summary; // Si no hi ha runs carregats, mostra tot
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 20 }}>
            {[
              ...
              { label: 'Millor latència',      value: bestLat ? `${bestLat.avgLatency?.toFixed(1)}ms` : '—', color: '#22c55e', Icon: IconZap },
          ...
          <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {sorted.map((s, i) => {
                    const isBest     = i === 0;
                    const platColor  = PLATFORM_COLORS[s.platform || s.broker] || 'var(--text-secondary)';
                    ...
                    return (
                      <tr key={i} style={{ ...S.tableRow, background: isBest ? 'rgba(34,197,94,0.04)' : 'transparent' }}>
                        ...
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {s.architecture
                            ? <span style={{ ...S.badge('#2563eb'), fontSize: 11 }}>{s.architecture}</span>
                            : <span style={{ color: 'var(--text-disabled)' }}>—</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {s.protocol
                            ? <span style={{ ...S.badge('#16a34a'), fontSize: 11 }}>{s.protocol}</span>
                            : <span style={{ color: 'var(--text-disabled)' }}>—</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {(s.platform || s.broker)
                            ? <span style={{ ...S.badge(platColor), fontSize: 11 }}>{s.platform || s.broker}</span>
                            : <span style={{ color: 'var(--text-disabled)' }}>—</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#f59e0b' }}>
                          {s.avgLatency?.toFixed(2) ?? '—'}<span style={{ fontSize: 10, fontWeight: 400, color: 'var(--text-disabled)' }}>ms</span>
                        </td>
                        <td style={{ ...S.td, textAlign: 'right', fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>
                          {s.p50Latency?.toFixed(2) ?? <span title="No disponible en agregació" style={{ color: 'var(--text-disabled)', cursor: 'help' }}>—</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'right', fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>
                          {s.p99Latency?.toFixed(2) ?? <span title="No disponible en agregació" style={{ color: 'var(--text-disabled)', cursor: 'help' }}>—</span>}
                        </td>
            ...
            <div style={{ padding: '10px 20px', borderTop: '1px solid var(--border)', fontSize: 11, color: 'var(--text-disabled)', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              ...
              <span>P50/P99: disponibles si el backend les retorna · <span style={{ fontStyle: 'italic' }}>—</span> = no disponible en mode agregació</span>
            </div>
          </div>
        </>
      )}
