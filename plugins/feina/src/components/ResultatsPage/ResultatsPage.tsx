...
const MetricGlossary = () => {
  ...
  const metrics = [
    { name: 'Latencia (ms)',     color: '#f59e0b', better: 'menor', desc: "Temps que tarda un missatge a viatjar des del productor fins al consumidor. Una latencia baixa es crucial per a aplicacions en temps real (trading, IoT, streaming).", example: 'Kafka pot assolir latencies de <10ms en condicions optimes.' },
    { name: 'Throughput (msg/s)',color: '#22c55e', better: 'major', desc: "Nombre de missatges processats per segon. Mesura la capacitat del sistema. Alt throughput es essencial per a big data i streaming d'alta frequencia.", example: 'Kafka pot superar el milio de msg/s en clusters optimitzats.' },
    { name: "Taxa d'error (%)",  color: '#ef4444', better: 'menor', desc: "Percentatge de missatges que fallen: no arriben, arriben tardanament o es corrompren. Afecta directament la fiabilitat del sistema.", example: "En produccio, una taxa d'error >0.1% ja es preocupant." },
    { name: 'P50 / Mediana',     color: '#3b82f6', better: 'menor', desc: 'El 50% dels missatges arriben en menys d\'aquest temps. Representa el rendiment tipic o normal del sistema.', example: 'Si P50=5ms, la meitat dels teus missatges s\'envien en menys de 5ms.' },
    { name: 'P99 (cua llarga)',  color: '#7c3aed', better: 'menor', desc: 'El 99% dels missatges arriben en menys d\'aquest temps. Mesura el pitjor cas practic. Critic per a SLA i contractes de servei.', example: 'Si P99=200ms pero P50=5ms, hi ha outliers que calen investigar.' },
  ];
  return (
    <div style={{ ...S.card, marginBottom: 20, borderLeft: '3px solid #3b82f6' }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Guia de metriques - Que estem comparant?</span>
        </div>
...
const HBarChart = ({ data, title, unit = '', color = '#3b82f6', showBest = false }: {
  data: { label: string; value: number }[]; title: string; unit?: string; color?: string; showBest?: boolean;
}) => {
  const [hovered, setHovered] = useState<number | null>(null);
  if (!data.length) return <div style={{ textAlign: 'center', color: 'var(--text-disabled)', padding: '20px 0', fontSize: 13 }}>Sense dades</div>;
            ...
            <div key={i} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
              ...
              <div style={{ flex: 1, height: 22, background: 'var(--bg-hover)', borderRadius: 4, position: 'relative', overflow: 'hidden' }}>
                <div style={{ width: `${pct}%`, height: '100%', background: isBest ? color : color + 'aa', borderRadius: 4, transition: 'width 0.5s cubic-bezier(0.4,0,0.2,1)', position: 'relative' }}>
                  {pct > 30 && <span style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', fontSize: 10, fontWeight: 700, color: 'white', fontFamily: 'var(--font-mono)', opacity: isHov ? 1 : 0.8 }}>{d.value.toFixed(1)}{unit}</span>}
                </div>
              </div>
...
const LiveLineChart = ({ data, color = '#3b82f6', label, unit = '' }: {
  ...
  if (data.length < 2) return <div style={{ textAlign: 'center', color: 'var(--text-disabled)', fontSize: 12, padding: '14px 0' }}>Esperant dades...</div>;
  const W = 480, H = 80;
...
const HistorialTab = () => {
  ...
  // P50/P99 calculats client-side a partir de les metriques raw
  useEffect(() => {
  ...
  const syncedSummary  = runs.length > 0 ? summary.filter(s => runScenarioIds.has(s.scenarioId)) : summary;
  ...
  const availDataFormats = [...new Set(syncedSummary.map((s: any) => scenarioMap[s.scenarioId]?.dataFormat || 'default').filter(Boolean))];
      ...
      <div style={{ ...S.card, marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={() => setFiltersOpen(o => !o)}
            ...
            Filtres
            {activeFilters > 0 && (
              <span style={{ background: 'var(--accent)', color: 'white', borderRadius: '50%', width: 18, height: 18, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700 }}>{activeFilters}</span>
            )}
            <IconChevron open={filtersOpen} />
          </button>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {filteredSummary.length !== syncedSummary.length && (
              <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Mostrant <strong>{filteredSummary.length}</strong> de {syncedSummary.length}</span>
            )}
            {activeFilters > 0 && (
              <button onClick={clearFilters} style={{ ...S.btn, fontSize: 12, padding: '4px 12px', color: 'var(--error)', borderColor: 'var(--error)' }}>Esborra tots</button>
            )}
          ...
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
            {availPlatforms.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {availPlatforms.map(p => <Chip key={p} label={p} active={filterPlatform.includes(p)} color="#d97706" onClick={() => toggle(filterPlatform, setFilterPlatform, p)} />)}
                </div>
              </div>
            )}
            ...
            {availArchs.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {availArchs.map(a => <Chip key={a} label={a} active={filterArch.includes(a)} color="#2563eb" onClick={() => toggle(filterArch, setFilterArch, a)} />)}
                </div>
              </div>
            )}
            {availDataFormats.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {availDataFormats.map(f => <Chip key={f} label={DATA_FORMAT_LABELS[f] || f} active={filterDataFormat.includes(f)} color="#7c3aed" onClick={() => toggle(filterDataFormat, setFilterDataFormat, f)} />)}
                </div>
              </div>
            )}
          </div>
      ...
      {filteredSummary.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div style={{ ...S.card }}><HBarChart data={latData}  title="Latencia mitjana (ms)"   unit="ms" color="#f59e0b" showBest /></div>
            <div style={{ ...S.card }}><HBarChart data={tputData} title="Throughput mitja (msg/s)" unit=""   color="#22c55e" /></div>
          </div>
          <div style={{ ...S.card, marginBottom: 20 }}>
          ...
          <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-disabled)' }}>{sorted.length} escenaris - ordenats per latencia</span>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={S.tableHeader}>
                    ...
                    <th style={{ ...S.th, textAlign: 'right' }}>Latencia avg</th>
                    ...
                    return (
                      <tr key={i} style={{ ...S.tableRow, background: isBest ? 'rgba(34,197,94,0.04)' : 'transparent' }}>
                        ...
                        <td style={{ ...S.td, fontWeight: 700, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          <div title={nameMap[s.scenarioId] || s.scenarioId}>{nameMap[s.scenarioId] || s.scenarioId?.slice(0, 12) || '-'}</div>
                          {dfLabel && <div style={{ fontSize: 10, color: 'var(--text-disabled)', fontWeight: 400 }}>{dfLabel}</div>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {s.architecture ? <span style={{ ...S.badge('#2563eb'), fontSize: 11 }}>{s.architecture}</span> : <span style={{ color: 'var(--text-disabled)' }}>-</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {s.protocol ? <span style={{ ...S.badge('#16a34a'), fontSize: 11 }}>{s.protocol}</span> : <span style={{ color: 'var(--text-disabled)' }}>-</span>}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {platform ? <span style={{ ...S.badge(platColor), fontSize: 11 }}>{platform}</span> : <span style={{ color: 'var(--text-disabled)' }}>-</span>}
                        </td>
    ...
    const poll = async () => {
      try {
        ...
        if (!data || !Array.isArray(data) || data.length === 0) {
          if (sel?.scenarioId) data = await fetch(`${METRICS_BASE}/metrics?scenarioId=${sel.scenarioId}`).then(r => r.json()).catch(() => null);
        }
        if (Array.isArray(data) && data.length > 0) { setMetrics(data.slice(-120)); setLastUpdate(new Date()); setPollError(''); }
      ...
      <div style={{ ...S.card, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Escenari en execucio</div>
            {activeRuns.length === 0
              ? <div style={{ color: 'var(--text-disabled)', fontSize: 14 }}>Cap escenari actiu.</div>
              : <select value={selectedRunId} onChange={e => setSelectedRunId(e.target.value)} style={{ ...S.input, maxWidth: 340 }}>
                  {activeRuns.map(r => <option key={r.id} value={r.id}>{r.scenarioName || r.id.slice(0, 14)}</option>)}
                </select>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: polling && activeRuns.length > 0 ? '#22c55e' : '#94a3b8', display: 'inline-block', boxShadow: polling && activeRuns.length > 0 ? '0 0 8px #22c55e80' : 'none', animation: polling && activeRuns.length > 0 ? 'pulseDot 2s ease infinite' : 'none' }}/>
            ...
            {lastUpdate && <span style={{ fontSize: 11, color: 'var(--text-disabled)' }}>- actualitzat {lastUpdate.toLocaleTimeString('ca-ES')}</span>}
          </div>
          {sel && (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              {sel.platform     && <span style={{ ...S.badge(PLATFORM_COLORS[normalizePlatform(sel.platform)] || '#7c3aed'), fontSize: 11 }}>{normalizePlatform(sel.platform)}</span>}
            </div>
          )}
        </div>
        {pollError && <div style={{ marginTop: 10, padding: '8px 12px', background: 'rgba(239,68,68,0.08)', border: '1px solid var(--error)', borderRadius: 6, fontSize: 12, color: 'var(--error)' }}>Error en carregar metriques: {pollError}</div>}
      </div>
      ...
      {activeRuns.length === 0 ? (
        <div style={{ ...S.card, textAlign: 'center', padding: 72 }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-secondary)', maxWidth: 380, margin: '0 auto 20px' }}>
            Inicia un escenari des de la pagina <strong>Escenaris</strong> i aqui apareixeran les metriques en temps real.
          </div>
          <a href="/escenaris" style={{ ...S.btnPrimary as React.CSSProperties, textDecoration: 'none', display: 'inline-flex' }}>Anar a Escenaris</a>
        </div>
      ) : (
        <>
          ...
          {metrics.length >= 5 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[
                { l: 'P50 Latencia (mediana)',    v: `${p50(lat)}ms`, c: '#3b82f6', desc: "50% dels missatges arriben en menys d'aquest temps" },
                { l: 'P99 Latencia (cua llarga)', v: `${p99(lat)}ms`, c: '#7c3aed', desc: "99% dels missatges arriben en menys d'aquest temps (pitjor cas practic)" },
              ].map(c => (
          ...
          {metrics.length > 0 && (
            <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '10px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-disabled)' }}>{metrics.length} punts - actualitzacio cada 3s</span>
              </div>
  ...
  return (
    <div style={{ ...S.page, maxWidth: 1200 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 15 }}>Metriques en temps real i comparatives d'arquitectures asincrones</p>
      </div>
      <div style={{ borderBottom: '1px solid var(--border)', marginBottom: 24, display: 'flex' }}>
        <button style={tabBtn(tab === 'live')}      onClick={() => setTab('live')}><IconPulse /> En directe</button>
        <button style={tabBtn(tab === 'historial')} onClick={() => setTab('historial')}><IconClock /> Historial i comparatives</button>
      </div>
      {tab === 'live'      && <LiveTab />}
