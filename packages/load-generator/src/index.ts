...
function snapshot(final = false) {
  ...
  return {
    // Fields expected by metrics-api /metrics/summary aggregation
    runId:        CFG.runId,
...
async function runKafka() {
  ...
  log(`Metrics  : ${CFG.metricsApiUrl}/metrics`);
  ...
  log(`=== Final: sent=${final.messages_sent} recv=${final.messages_recv} latency=${final.latency}ms throughput=${final.throughput} msg/s ===`);
